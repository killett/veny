# Wiring index — phase 3e (`pipeline.py`, `cli.py`, `run_options.py`)
...
which is what made its headline false.** Of **250 substitutions** covering
**218 distinct (call site, argument) pairs** across **99 call-site groups**,
**202 kill at least one named test, 8 are identity substitutions that cannot
kill anything, and 40 remain OPEN HOLE** — every one of the 40 named below with
the reason it cannot be closed. Before task 8 the same 250 substitutions scored
**136 pinned / 106 holes / 8 identity**: task 8 closed **66** of the 106.
...
- **`bool` — both values, always.** Every boolean argument appears twice, once
  ...
  no-op wherever the default is already `False`. 65 of the 250 rows are one
  half of such a pair.
- **empty value of the same type** — `set()`, `{}`, `[]`, `()`, `frozenset()`,
  `None`.
- **wrong-but-type-correct**, where no natural empty value exists —
  `Path("/tmp/wrong-venv")`, `Path("/tmp/wrong-script.py")`, `"wrongname"`,
  `"wrongflag"`, `"9.9"`, `"20000101-000000"`, `run_options.Options()`,
  `object()`, `stdlib_index.StdlibIndex(names=frozenset(), …)`,
  `alias_index.empty(Path("/tmp/wrong-index"))`, `contextlib.nullcontext(…)`,
  `lambda: None`. Each is recorded in the table, so the evidence is auditable.
...
**Rows marked *identity*** are the eight where the substituted literal is
...
`pipeline.py:909`, and three getattr defaults of `False`). They are listed
...
prepended copy and would test the unmutated source. Two full sweeps were run —
one at `183bdcc` before any test was added, one at `4033c75` after — plus
targeted re-runs after each batch. The raw per-substitution results are in
`.superpowers/sdd/2026-08-19-pipeline-and-cli-slimming/task-8-report.md`
(untracked).

Re-run this whole check after any further extraction; a row whose test no
longer dies is a hole, not a stale row.

## Scope: what counts as a call site here

`pipeline.py` and `cli.py` together hold **221 call expressions with at least
one argument, carrying 458 arguments**. The sweep covers the **99 groups /
218 arguments** that cross a boundary — a call into another veny module
(`environment.*`, `verify.*`, `cache_search.*`, `alias_index.*`, `last_used.*`,
`venv_cache.*`, `stdlib_index.*`, `classify.*`, `analysis_scan.*`,
`custom_modules.*`, `pipeline.*`), into emmykit (`ek.*`), into the process or
filesystem (`subprocess.run`, `shutil.rmtree`, `mkdir`), a value class that
crosses a layer (`Settings`, `ImportScan`, `ResolvedImport`), a function this
phase moved into `pipeline.py` and now calls by name, `options.set_venv_dir`,
and every `getattr(options.args, …)` argv read.

The remaining **121 calls / 222 arguments are excluded, and here is the whole
list of why**, adding to 121 exactly: **51** `logging.*` (48) and `print` (3)
calls — message formatting, not wiring; phase 3d excluded them too. **17**
`parser.add_argument` calls — argparse spec, untouched by this phase and
covered by `tests/test_options_surface.py`. **6** `getattr(options.args, …)`
argv reads that appear as an argument of a call already swept, where
substituting the enclosing argument with `True`/`False` subsumes a mis-read
flag name. **17** pure conversions of a value swept at the enclosing site or
nested inside an excluded `logging` call — `os.fspath`/`os.sep` (7),
`file.*`/`directory.*`/`sys.*`/`re.match`/`shlex.quote`/`" ".join` (10). And
**30** builtin (`len`, `set`, `str`, `tuple`, `sorted`, `list`, `map`, `any`,
`sum`, `isinstance`, `type`, `hasattr`, `argparse.REMAINDER`) and
exception-constructor (`SystemExit`, `ValueError`, `FileNotFoundError`,
`UsageError`, `VenvBuildFailed`) calls. Nothing was excluded for being
...
one thing: `--debug` reaching `log_level`, which also closes the `"debug"`
argv read at `cli.py:156`.

## Findings the sweep produced beyond the pins

**17 of the 40 open-hole rows — 13 distinct arguments at six call sites — are
not test gaps at all.** They are **dead arguments**: values built at a call site
that the callee never reads, or that have no effect there. No test can pin them,
and they should be deleted rather than tested:
...
- `pipeline.py:222` `ImportScan(loaded_custom_modules=…, samedir_files=…,
  subfolders=…, sys_path_hints=…, seen_stdlib_imports=…)` — `classify.split_imports`
  reads only `all_imports`, `custom_modules` and `seen_stdlib_imports`, and
  `pipeline.split_imports` copies back only four result fields, so even
  `seen_stdlib_imports` is unobservable here.
...
The other 23 rows break down as: **4** argparse defaults that argparse never
lets a real command line reach (`script_args`, `rawlog`, `debug`, `script`);
**6** where the callee is stubbed in every run that reaches it because the real
one is an emmykit, subprocess or filesystem boundary (`ek.print_all_errors` ×3,
`ek.save_options_to_json`, `stdlib_index.resolve`, `build_alias_index`); **5**
branches no run can construct (`raise_on_empty`/`strict` on a path argparse
cannot produce, `ignore_errors` on a directory `pipeline.run` creates a few
lines earlier, `parents`/`exist_ok` on a directory just proven absent under an
always-present `$HOME`); **3** `use_reqs` positionals that are no-ops wherever
they are not identity; and **5** where the substitution is indistinguishable
from the real value.

Two more are pre-existing defects the sweep surfaced rather than gaps:

- `pipeline.py:483` `getattr(options.args, "y", False)` — argparse gives
  `-y/--yes` the dest `yes`, so this read is **already always False** and `"y"`
  and `"wrongflag"` are indistinguishable. Recorded, not fixed: this phase is
...
`3e sweep 1` marks the rows that were holes at `183bdcc`, before task 8 added a
test. Test names are abbreviated: `test_cli_entry_point::x` means
`tests/test_cli_entry_point.py::x`. Where more than three named tests die, the
first three are listed with a count of the rest. Line numbers are as of
`4033c75`.

| Site | Argument | Substituted with | Test that died | 3e sweep 1 |
|---|---|---|---|---|
...
| `cli.py:184` (main) -> `pipeline.run` | `start_time` | `dt.datetime(2000, 1, 1)` | **OPEN HOLE** -- `start_time` feeds nothing but the two "Elapsed time: <delta>" lines. Pinning it would mean asserting a measured duration, which pins a clock rather than the wiring. | was a hole |
...
| `pipeline.py:125` (find_imports_in_script) -> `Settings` | `my_name` | `"wrongname"` | **OPEN HOLE** -- Dead argument: `analysis.scan` reads only `settings.rawlog` off the Settings it is handed. `my_name` is never read at this call site. | was a hole |
| `pipeline.py:125` (find_imports_in_script) -> `Settings` | `cwd` | `Path("/tmp/wrong-dir")` | **OPEN HOLE** -- Dead argument: `analysis.scan` reads only `settings.rawlog`. `cwd` is never read at this call site. | was a hole |
| `pipeline.py:125` (find_imports_in_script) -> `Settings` | `stay_out_list` | `()` | **OPEN HOLE** -- Dead argument: `analysis.scan` reads only `settings.rawlog`. `stay_out_list` is never read at this call site. | was a hole |
| `pipeline.py:125` (find_imports_in_script) -> `Settings` | `search_above_this_dir` | `True` | **OPEN HOLE** -- Dead argument: `analysis.scan` reads only `settings.rawlog`. `search_above_this_dir` is never read at this call site. | was a hole |
| `pipeline.py:125` (find_imports_in_script) -> `Settings` | `search_above_this_dir` | `False` | **OPEN HOLE** -- Dead argument: `analysis.scan` reads only `settings.rawlog`. `search_above_this_dir` is never read at this call site. | was a hole |
...
| `pipeline.py:201` (is_importable) -> `alias_index.ResolvedImport` | `pip_name` | `"wrongname"` | **OPEN HOLE** -- Dead argument: `check_packages_in_venv`'s single-record branch checks `record.import_name` and nothing else. The probe supplies `pip_name=import_name` only because ResolvedImport requires both. | was a hole |
...
| `pipeline.py:222` (split_imports) -> `ImportScan` | `loaded_custom_modules` | `set()` | **OPEN HOLE** -- Dead argument: `classify.split_imports` reads only `scan.all_imports`, `scan.custom_modules` and `scan.seen_stdlib_imports`. | was a hole |
| `pipeline.py:222` (split_imports) -> `ImportScan` | `samedir_files` | `[]` | **OPEN HOLE** -- Dead argument: `classify.split_imports` reads only `scan.all_imports`, `scan.custom_modules` and `scan.seen_stdlib_imports`. | was a hole |
| `pipeline.py:222` (split_imports) -> `ImportScan` | `subfolders` | `[]` | **OPEN HOLE** -- Dead argument: `classify.split_imports` reads only `scan.all_imports`, `scan.custom_modules` and `scan.seen_stdlib_imports`. | was a hole |
| `pipeline.py:222` (split_imports) -> `ImportScan` | `sys_path_hints` | `set()` | **OPEN HOLE** -- Dead argument: `classify.split_imports` reads only `scan.all_imports`, `scan.custom_modules` and `scan.seen_stdlib_imports`. | was a hole |
| `pipeline.py:222` (split_imports) -> `ImportScan` | `seen_stdlib_imports` | `set()` | **OPEN HOLE** -- Effectively dead: `classify.split_imports` returns it as `Requirements.seen_stdlib`, but `pipeline.split_imports` copies back only all_imports, bad, uninstalled and total_imports, so nothing downstream can observe it. | was a hole |
...
| `pipeline.py:451` (feeling_lucky) -> `run_script` | `rawlog` | `True` | **OPEN HOLE** -- No effect at this call site: `run_script` reads `rawlog` only to guard the announce line, and this site passes `announce=False`. | was a hole |
| `pipeline.py:451` (feeling_lucky) -> `run_script` | `rawlog` | `False` | **OPEN HOLE** -- No effect at this call site: `run_script` reads `rawlog` only to guard the announce line, and this site passes `announce=False`. | was a hole |
...
| `pipeline.py:768` (run) -> `blank_slate` | `positional 0` | `run_options.Options()` | **OPEN HOLE** -- A fresh Options built under the same HOME and working directory carries the same my_dir, my_name and cwd, and its empty args Namespace makes the -y read False exactly as the real one does (see the dest bug above), so the substitution is indistinguishable from the real argument. A distinguishing one would need an Options whose my_dir differs, which main() gives a test no way to inject. | was a hole |
...
| `pipeline.py:788` (run) -> `Settings` | `stay_out_list` | `()` | **OPEN HOLE** -- `dict_of_custom_modules` consults stay_out_list only through `is_standard_path`, and the test that drives the real scan narrows sys.path to a temporary directory that matches no entry either way. | was a hole |
...
| `pipeline.py:821` (run) -> `run_script` | `rawlog` | `True` | **OPEN HOLE** -- No effect at this call site: `run_script` reads `rawlog` only to guard the announce line, and this site leaves `announce` at its False default. | was a hole |
| `pipeline.py:821` (run) -> `run_script` | `rawlog` | `False` | **OPEN HOLE** -- No effect at this call site: `run_script` reads `rawlog` only to guard the announce line, and this site leaves `announce` at its False default. | was a hole |
...
| `pipeline.py:844` (run) -> `run_script` | `rawlog` | `True` | **OPEN HOLE** -- No effect at this call site: `run_script` reads `rawlog` only to guard the announce line, and this site leaves `announce` at its False default. | was a hole |
| `pipeline.py:844` (run) -> `run_script` | `rawlog` | `False` | **OPEN HOLE** -- No effect at this call site: `run_script` reads `rawlog` only to guard the announce line, and this site leaves `announce` at its False default. | was a hole |
...
## The 40 remaining OPEN HOLEs, in one place

Every one is listed in the table above with the same reason; this is the
roll-up, so the count can be audited without reading 250 rows. None of the
40 is a case of "a test could see this and none does" left unwritten out of
convenience: each reason below is a dead argument, an unreachable branch, an
effect emmykit owns, or a substitution the code makes indistinguishable from
the real value.

| Site | Argument | Substituted with | Why it cannot be closed |
|---|---|---|---|
...
| `cli.py:184` (main) -> `pipeline.run` | `start_time` | `dt.datetime(2000, 1, 1)` | `start_time` feeds nothing but the two "Elapsed time: <delta>" lines. Pinning it would mean asserting a measured duration, which pins a clock rather than the wiring. |
...
| `pipeline.py:768` (run) -> `blank_slate` | `positional 0` | `run_options.Options()` | A fresh Options built under the same HOME and working directory carries the same my_dir, my_name and cwd, and its empty args Namespace makes the -y read False exactly as the real one does (see the dest bug above), so the substitution is indistinguishable from the real argument. A distinguishing one would need an Options whose my_dir differs, which main() gives a test no way to inject. |
| `pipeline.py:788` (run) -> `Settings` | `stay_out_list` | `()` | `dict_of_custom_modules` consults stay_out_list only through `is_standard_path`, and the test that drives the real scan narrows sys.path to a temporary directory that matches no entry either way. |
