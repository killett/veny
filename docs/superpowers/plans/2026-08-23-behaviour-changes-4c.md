...
- **CORRECTION (2026-08-23, Task 10 review):** the bullet above overstates it — 4a already made two sanctioned behaviour changes of its own, by user ruling (folder scanning deleted; a missing script turned into a usage error, exit status 2), both recorded in `PROGRESS.md`'s phase-4a entries. What is true and unique to 4c is narrower: it is the first phase whose *purpose* is behaviour change rather than structure. Left standing above as the plan's own history at the time it was written; do not repeat the overstated framing in later plans.
