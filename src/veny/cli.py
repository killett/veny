...
from .analysis.literals import collect_pathlib_aliases
...
def _analyze_module(
    ...
    # ImportFunctionCollector only reads/writes custom_modules; aliasing it
    # into a throwaway ImportScan (same dict, not a copy) means writes still
    # land on options.custom_modules with no copy-back needed.
    collector_scan = ImportScan(custom_modules=options.custom_modules)
    collector = ImportFunctionCollector(collector_scan, module_key, module_path)
...
def _enqueue_top_level_imports(
    ...
    """
    # process_import now takes an ImportScan rather than Options. These are
    # the same objects options already holds, not copies, so mutations made
    # through scan land on options too -- no copy-back needed.
    scan = ImportScan(
    ...
    for import_name in import_names:
        ...
        resolved = process_import(
            scan, import_name, module_path, is_stdlib=options.stdlib.__contains__
        )
        if not resolved:
...
def find_imports_in_script(
    ...
    if not ek.is_python_script(first_path) or not ek.compile_code(first_path):
        ...
        return
    # These are the same objects options already holds, not copies, so
    # mutating through scan mutates what options exposes -- no copy-back
    # needed. Task 5 replaces this bridge once the orchestrator itself moves
    # out of cli.py.
    scan = ImportScan(
        all_imports=options.all_imports,
        custom_modules=options.custom_modules,
    ...
    while new_modules_found:
        ...
        for import_name in used_imports.copy():
            ...
            process_import(
                scan, import_name, first_path, is_stdlib=options.stdlib.__contains__
            )
